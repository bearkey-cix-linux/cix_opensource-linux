...
struct csi_dma_dev *csi_dma_dev_info[4];

struct csi_dma_dev *get_csi_dma_dev_handler(int no)
{
	if (no > 3)
		return NULL;

	return csi_dma_dev_info[no];
}

int set_csi_dma_dev_handler(struct csi_dma_dev *handler,int no)
{
	if (no > 3)
		return -1;

	csi_dma_dev_info[no] = handler;
	return 0;
}

static struct dentry *csi_dma_debug_dir;
static int sync_mode = 1;  /*sync mode*/
static int irq_cnt = 0;  /*debug irq */
...
{
	struct csi_dma_dev *csi_dma = get_csi_dma_dev_handler(0);

	/*change the sync mode*/
	csi_dma->mode = sync_mode;
}

static ssize_t sync_mode_read(struct file *file, char __user *user_buf,
...
static ssize_t sync_mode_write(struct file *file, const char __user *user_buf,
		size_t count, loff_t *ppos)
{
	char buf[16];
	int val;
...
static const struct file_operations sync_mode_fops = {
	.read  = sync_mode_read,
	.write = sync_mode_write,
};

void set_irq_cnt(void)
{
	struct csi_dma_dev *csi_dma = get_csi_dma_dev_handler(0);

	/*change the sync mode*/
	csi_dma->irq_cnt = irq_cnt;
}

static ssize_t irq_debug_cnt_read(struct file *file, char __user *user_buf,
		size_t count, loff_t *ppos)
{
	char buf[8];
	int len;
...
static ssize_t irq_debug_cnt_write(struct file *file, const char __user *user_buf,
		size_t count, loff_t *ppos)
{
	char buf[16];
	int val;
...
static const struct file_operations irq_cnt_fops = {
	.read  = irq_debug_cnt_read,
	.write = irq_debug_cnt_write,
};

static int csi_dma_debug_init(void)
{
	csi_dma_debug_dir = debugfs_create_dir("csi_dma", NULL);
	if (IS_ERR(csi_dma_debug_dir))
		return PTR_ERR(csi_dma_debug_dir);

	if (!debugfs_create_file("sync_mode", 0644, csi_dma_debug_dir, NULL, &sync_mode_fops)) {
		debugfs_remove_recursive(csi_dma_debug_dir);
		return -ENOMEM;
	}

	if (!debugfs_create_file("irq_debug_cnt", 0644, csi_dma_debug_dir, NULL, &irq_cnt_fops)) {
		debugfs_remove_recursive(csi_dma_debug_dir);
		return -ENOMEM;
	}

	pr_info("csi dma debug initialized \n");

	return 0;
}

static void csi_dma_debug_deinit(void)
{
	debugfs_remove_recursive(csi_dma_debug_dir);
	pr_info("csi dma debug exit \n");
}
...
static int csi_dma_parse(struct csi_dma_dev *csi_dma)
{
	...
	/*here need get mode single (0) or multi mode (1,2,3)*/
	csi_dma->mode = get_sync_mode();
	csi_dma->picture_position = PICTURE_POSITION_RIGHT;
...
static int csi_dma_probe(struct platform_device *pdev)
{
	...
	set_csi_dma_dev_handler(csi_dma,csi_dma->id);
