...
inline void csi_dma_write(struct csi_dma_dev *csi_dma, u32 reg, u32 val)
{
	writel(val, csi_dma->regs + reg);
}

void csi_dma_set_output_buffer(struct csi_dma_dev *csi_dma,u64 base_addr,u64 offset_addr)
{
				...
				struct csi_dma_buffer *buf)
{
	...
	/*slave buffer updata*/
	{
		struct csi_dma_dev *csi_dma_slave = get_csi_dma_dev_handler(2);

		/*here use master width,for yuv422*/
		if (csi_dma->picture_position == PICTURE_POSITION_RIGHT) {
			csi_dma_set_output_buffer(csi_dma_slave,paddr->y,0);
			csi_dma_set_output_buffer(csi_dma,paddr->y,csi_dma->dma_cap->src_f.width);
		} else {
			csi_dma_set_output_buffer(csi_dma_slave,paddr->y,csi_dma->dma_cap->src_f.width);
			csi_dma_set_output_buffer(csi_dma,paddr->y,0);
		}
	}
...
//	csi_dma_write(csi_dma, AXI_USER0, val);
	csi_dma_write(csi_dma, AXI_USER0, 0x21);
				 ...
				 struct csi_dma_frame *src_f)
{
	...
	if(csi_dma->mode == 1) {
		/*2 camera*/
		val = (src_f->width/2 << FRAME_NUM_OFFSET) |
