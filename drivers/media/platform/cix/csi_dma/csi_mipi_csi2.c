...
struct csi2rx_priv *csi2rx_info[4];

struct csi2rx_priv *get_mipi_csi2_handler(int no)
{
	if (no > 3)
		return NULL;

	return csi2rx_info[no];
}

int set_mipi_csi2_handler(struct csi2rx_priv *handler,int no)
{
	if (no > 3)
		return -1;

	csi2rx_info[no] = handler;
...
static int mipi_csi2_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		/*slave*/
		{
			struct csi2rx_priv *csi2rx_slave = get_mipi_csi2_handler(2);
			pm_runtime_get_sync(csi2rx_slave->dev);
		}
		...
		{
			struct csi2rx_priv *csi2rx_slave = get_mipi_csi2_handler(2);
			pm_runtime_put(csi2rx_slave->dev);
		}
...
static int mipi_csi2_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		if (!csi2rx->count) {
			...
			{
				struct csi2rx_priv *csi2rx_slave = get_mipi_csi2_handler(2);
				//mipi_csi2_enable_err_irq(csi2rx_slave);
				mipi_csi2_stream_start(csi2rx_slave);
			}
		...
		if (!csi2rx->count) {
			...
			{
				struct csi2rx_priv *csi2rx_slave = get_mipi_csi2_handler(2);
				//mipi_csi2_disable_err_irq(csi2rx_slave);
				mipi_csi2_stream_stop(csi2rx_slave);
			}
...
static int mipi_csi2_probe(struct platform_device *pdev)
{
	...
	set_mipi_csi2_handler(csi2rx,csi2rx->id);
